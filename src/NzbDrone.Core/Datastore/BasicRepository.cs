...
namespace NzbDrone.Core.Datastore
{
    ...
    public class BasicRepository<TModel> : IBasicRepository<TModel> where TModel : ModelBase, new()
    {
        ...
        {
            return Query<IEnumerable<TModel>>((q => q.ToList()));
        }
        ...
        {
            TModel model = Query<TModel>(q => q.Where(c => c.Id == id).SingleOrDefault());
        ...
        public IEnumerable<TModel> Get(IEnumerable<int> ids)
        {
            ...
            var result = Query<List<TModel>>(q => q.Where(m => m.Id.In(idList)).ToList());
        ...
        {
            pagingSpec.Records = Query<List<TModel>>(q => GetPagedQuery(q, pagingSpec).Skip(pagingSpec.PagingOffset())
