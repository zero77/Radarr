...
namespace NzbDrone.Core.Messaging.Commands
{
    ...
    public class CommandRepository : BasicRepository<CommandModel>, ICommandRepository
    {
        ...
        {
            return Query<List<CommandModel>>(q => q.Where(c => c.Name == name).ToList());
        }

        public List<CommandModel> FindQueuedOrStarted(string name)
        {
            return Query<List<CommandModel>>(q => q.Where(c => c.Name == name)
        ...
        {
            return Query<List<CommandModel>>(q => q.Where(c => c.Status == CommandStatus.Queued).ToList());
        }

        public List<CommandModel> Started()
        {
            return Query<List<CommandModel>>(q => q.Where(c => c.Status == CommandStatus.Started).ToList());
        }
